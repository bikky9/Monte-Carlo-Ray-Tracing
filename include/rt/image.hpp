...
namespace rt
{
	...
	public:
		/// Constructor.
		image_t(int width, int height, int samples , color_t _bgc);
		/// Destructor.
		virtual ~image_t();
		...
		///Returns No of samples per pixel
		int get_no_of_samples(void) const;
		...
		std::vector<Eigen::Vector2f> sample_pixels(unsigned int _x, unsigned int _y) const;
