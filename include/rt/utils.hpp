...
namespace rt
{
	...
	inline Eigen::Vector3d SampleFromHemisphere(double u, double v){
		double sintheta = sqrt(1-u*u);
		double phi = 2*M_PI*v;
		return Eigen::Vector3d(sintheta*cos(phi),u,sintheta*sin(phi));
	}
	inline void createCoordinateSystem(const Eigen::Vector3d &N, Eigen::Vector3d &Nt, Eigen::Vector3d &Nb) //taken from scratchpixel
	{
		if(abs(N[0]) > abs(N[1])){
			Nt = Eigen::Vector3d(N[2], 0, N[0] * -1) / sqrt(N[0] * N[0] + N[2] * N[2]);
		}else{
			Nt = Eigen::Vector3d(0, -1*N[2], N[1]) / sqrt(N[1] * N[1] + N[2] * N[2]);
		}
		Nb = N.cross(Nt);
	}
}
